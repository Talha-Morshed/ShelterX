-- ShelterX - JOIN Operations
-- Tables: facilities, users, services, facility_services, reviews, donations, volunteers, emergency_contacts
...
-- Q1: List ALL facilities and their reviews (including facilities with zero reviews).
...
-- Q2: List ALL users and their donation history (including users who have never donated).
...
-- Q3: List ALL facilities and how many volunteers each has (including facilities with zero volunteers).
...
-- Q4: List ALL services and which facilities offer them (including services not offered anywhere yet).
...
-- Q5: List ALL facilities and their primary emergency contact (including facilities that have none).
...
-- Q1: List ALL users and any reviews they've written (including users who have never written a review).
SELECT r.review_id, r.rating, r.comment,
...
-- Q2: List ALL services and the facilities that use them (including services not assigned to any facility).
SELECT f.facility_name, f.city,
...
-- Q3: List ALL facilities and any donations received (including facilities with zero donations).
...
-- Q4: List ALL users and their volunteer status (including users who are not volunteers).
SELECT v.volunteer_id, v.role, v.status,
       u.user_id, u.full_name, u.email
FROM   volunteers v
...
-- 4. FULL JOIN (FULL OUTER JOIN) simulated with UNION of LEFT JOIN + RIGHT JOIN

-- Q1: FULL JOIN between users and reviews (show ALL users AND ALL reviews, matched where possible).
...
-- Q2: FULL JOIN between facilities and donations (every facility and every donation, matched where possible).
...
-- BONUS: Multi-table complex JOINs

-- B1: Facility dashboard -- name, city, total donations, average rating, volunteer count (all in one query).
...
-- B2: Find facilities that offer Medical services (demonstrates filtering on JOINed table).
...
-- B3: Find users who are both donors AND volunteers (INTERSECT-like logic with INNER JOIN).
